...
samples_folder_name_context = 'images_with_context_%d_%.2f' % (
    temporal_size, detection_threshold)
optical_flow_folder_name = 'optical_flow_%d_%.2f' % (
    temporal_size, detection_threshold)
...
# 'conv3d_4_tasks_0.5_mae_wide_deep_resnet_3_5_losses_0.5_obj'  # % temporal_size #  'conv3d_context_slim_%d_2' % temporal_size
CHECKPOINTS_PREFIX = 'conv3d_4_tasks_0.5_mae_wide_deep_resnet_3_obj_relu_resnet'
CHECKPOINTS_BASE = os.path.join(
    output_folder_base, database_name, "checkpoints", CHECKPOINTS_PREFIX)
